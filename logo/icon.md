...
at the center gap — not a semicircle plus a rectangular stem. The stem falls out
...
the chord's offset from the center; all four sweeps are under 180°, so
...
baseline y=216.44, ink box x 26.417..243.583 centered on x=135.
...
3. Inkscape's `text-anchor` + `textLength` handling leaves the result off-center,
   so re-center it explicitly: query the bbox with
